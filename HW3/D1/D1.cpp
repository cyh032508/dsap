...
template<typename T>
class NumericArray : public IList<T> {
public:
    NumericArray() = default;
    NumericArray(std::initializer_list<T> l) : repr_{l}{}
    NumericArray(size_t count, const T& val) : repr_(count, val) {}

    T& operator[](size_t pos) override { return repr_[pos]; }
    const T& operator[](size_t pos) const override { return repr_[pos]; }
    void Insert(size_t pos, const T& val) override { repr_.Insert(pos, val); }
    void Insert(size_t pos, T&& val) override { repr_.Insert(pos, std::move(val)); }
    void Erase(size_t pos) override { repr_.Erase(pos); }
    size_t Size() const override { return repr_.Size(); }
    bool Empty() const override { return repr_.Empty(); }
    void Remove(const T& val) override { return repr_.Remove(val); }
...
T NumericArray<T>::Median() const {
    ...
    sortedArray.Sort();
    T median = sortedArray.repr_[Size() - 1];
    ...
    if(size % 2 == 0) {
        ...
        for(size_t j = Size() - 1; j >= 0; j--) {
            if(i == size/2)
                break;
            if(sortedArray.repr_[j] < median) {
                median = sortedArray.repr_[j];
                i++;
            }
        }
    } else {
...
NumericArray<T>& NumericArray<T>::Sort() {
    ...
    for (size_t i = 0; i < Size() - 1; ++i) {
        ...
        for (size_t j = i + 1; j < Size(); ++j) {
            if (repr_[j] < repr_[minIndex]) {
                minIndex = j;
            }
        }
        if (minIndex != i) {
            T temp = repr_[i];
            repr_[i] = repr_[minIndex];
            repr_[minIndex] = temp;
        }
    }
...
void NumericArray<T>::TrimZeroes() {
    ...
    for(int i = 0; i < Size() - (front + end); i++) {
        repr_[i] = repr_[front + i];
    }
    repr_.Resize(Size() - (front + end));
}
